...
cv::Mat_<float> ApplyKernel(const cv::Mat_<float> &mat, Kernel kernel_type,
                            int stride = 1, int padding = 0,
                            bool transpose = false);
cv::Mat_<float> CleanupEdges(const cv::Mat_<float> &image,
                             float accept_threshold, float consider_threshold);
cv::Mat_<float> Convolve(const cv::Mat_<float> &mat,
                         const cv::Mat_<float> &kernel, int stride = 1,
                         int padding = 0);
inline float FrobeniusInner(const cv::Mat_<float> &a,
